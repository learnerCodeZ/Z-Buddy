...
**Z-Buddy 插件即按此模板构建**（结构对齐官方 Example Plugin 推荐布局；模板还是 Phase 0 的实验台——临时替换其 hook 脚本完成了事件捕获与暂停验证，实验后已字节级还原）：

```
example-plugin（官方模板）          z-buddy（我们的插件）
├── .zcode-plugin/plugin.json  →  ├── .zcode-plugin/plugin.json
├── hooks/hooks.json           →  ├── hooks/hooks.json（7 事件全挂载）
├── hooks/pre-tool-use.mjs     →  ├── hooks/report.mjs
├── skills/example-skill/      →  └── skills/（hatch 技能预留位）
├── mcp/hello-server.mjs       →     （操作权证 MCP 预留位）
└── commands/hello.md          →     （/zpet 命令预留位）
```
