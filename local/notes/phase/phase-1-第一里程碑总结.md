...
## 0.8 第四里程碑：托盘 + 宠物切换持久化（同日追加）

1. **系统托盘**：托盘图标 + 菜单（显示/隐藏宠物 · 切换宠物 · 退出），tauri `tray-icon` feature；图标默认进溢出区（时钟旁 ^ 展开可见）；
2. **宠物切换持久化**：偏好存 `~/.z-buddy/app.json`（`{"pet":"matcha"}`），启动时 `get_pet_pref_cmd` 读取 → 加载对应包（内置 mochi / 外部包）；托盘"切换宠物"按列表循环并广播 `pet-changed` 事件热切换；实测：app.json 设 matcha → 重启后加载绿色抹茶 ✓；
