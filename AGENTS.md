...
- 思考行额外做了**手部左右微动**（像在搓下巴）：姿势按"手部矩形"拆成 body + hand 两层，
  hand 每帧**左右**位移 ±1.8px，body 被挖掉的矩形用上下边缘插值补掉（避免留洞）。
  ⚠️ 矩形**只许框到手、不能碰下巴** —— 框进下巴会让下巴跟着动（踩过这个坑，
  验证方法：第 2 帧与第 6 帧呼吸同相、手位移相反，除手部外应逐像素一致）；
  带气泡的状态行角色高度统一 140px（给气泡留空间）。
